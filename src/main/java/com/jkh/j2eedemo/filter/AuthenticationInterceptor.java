...
import com.jkh.j2eedemo.util.TokenUtil;
...
public class AuthenticationInterceptor implements HandlerInterceptor {
    ...
    public AuthenticationInterceptor(IGxyh userservice) {
        ...
        if (method.isAnnotationPresent(UserLoginToken.class)) {
            ...
            if (userLoginToken.required()) {
                ...
                try {
                    ...
                    throw new NoTokenException("401");
                }
                Gxyh gxyh = new Gxyh();
                System.out.println("获取到tokenid"+userId);
                gxyh.setZh(userId);
                Gxyh user = userservice.login(gxyh);
                if (user == null) {
                    throw new NoTokenException("用户不存在，请重新登录");
                }
                // 验证 token
                JWTVerifier jwtVerifier = JWT.require(Algorithm.HMAC256(user.getMm())).build();
                try {
